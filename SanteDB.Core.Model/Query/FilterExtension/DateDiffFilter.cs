        ...
        public BinaryExpression Compose(Expression scope, ExpressionType comparison, Expression valueExpression, Expression[] parms)
        {
            ...
            if (scope.Type.StripNullable() == typeof(DateTimeOffset))
            {
                ...
                return Expression.MakeBinary(comparison,
                    Expression.Call(exm, scope, Expression.Convert(parms[0], typeof(DateTimeOffset))),
                    valueExpression);
            }
            else
            {
                return Expression.MakeBinary(comparison,
                    Expression.Call(this.ExtensionMethod, scope, Expression.Convert(parms[0], typeof(DateTime))),
                    valueExpression);
            }
        }
    }
